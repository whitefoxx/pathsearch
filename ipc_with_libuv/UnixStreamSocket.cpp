    ...
    : bufferSize(bufferSize), role(role)
{
    ...
    if (err != 0)
    {
        ...
        THROW_ERROR_STD("uv_pipe_init() failed: %s", uv_strerror(err));
    }
    ...
    if (err != 0)
    {
        ...
        THROW_ERROR_STD("uv_pipe_open() failed: %s", uv_strerror(err));
    }
    ...
    if (this->role == UnixStreamSocket::Role::CONSUMER)
    {
        ...
        if (err != 0)
        {
            ...
            THROW_ERROR_STD("uv_read_start() failed: %s", uv_strerror(err));
        }
    }
...
void UnixStreamSocket::Close()
{
    ...
    if (this->role == UnixStreamSocket::Role::CONSUMER)
    {
        ...
        if (err != 0)
            ABORT("uv_read_stop() failed: %s", uv_strerror(err));
    }
    ...
    if (this->role == UnixStreamSocket::Role::PRODUCER && !this->hasError && !this->isClosedByPeer)
    {
        ...
        if (err != 0)
            ABORT("uv_shutdown() failed: %s", uv_strerror(err));
    }
...
void UnixStreamSocket::Write(const uint8_t *data, size_t len)
{
    ...
    {
        ERROR_STD("uv_try_write() failed, trying uv_write(): %s", uv_strerror(written));
    ...
    if (err != 0)
    {
        ERROR_STD("uv_write() failed: %s", uv_strerror(err));
...
inline void UnixStreamSocket::OnUvReadAlloc(size_t /*suggestedSize*/, uv_buf_t *buf)
{
    ...
    else
    {
        ...
        ERROR_STD("no available space in the buffer");
    }
}
...
inline void UnixStreamSocket::OnUvRead(ssize_t nread, const uv_buf_t * /*buf*/)
{
    ...
    {
        ERROR_STD("read error, closing the pipe: %s", uv_strerror(nread));
...
inline void UnixStreamSocket::OnUvWriteError(int error)
{
    ...
    ERROR_STD("write error, closing the pipe: %s", uv_strerror(error));
